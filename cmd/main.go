...
func generateDefaultConfig() {
	...
	fmt.Println("  voice_id               - 语音引擎和声音，如 edge:zh-CN-YunjianNeural")
	fmt.Println("  template               - 语音模板数组（推荐配置方式）")
	fmt.Println("  prefix_text            - 开头语音文本（旧版兼容字段）")
	fmt.Println("  suffix_text1           - 结尾语音文本1（旧版兼容字段）")
	fmt.Println("  suffix_text2           - 结尾语音文本2（旧版兼容字段）")
	fmt.Println("  silence_start          - 开始前静音秒数（旧版兼容字段）")
	fmt.Println("  silence_after_prefix   - 前缀后静音秒数（旧版兼容字段）")
	fmt.Println("  silence_after_main     - 主文本后静音秒数（旧版兼容字段）")
	fmt.Println("  silence_after_suffix   - 后缀1后静音秒数（旧版兼容字段）")
	fmt.Println("  silence_end            - 结束后静音秒数（旧版兼容字段）")
	fmt.Println("  screenshot_before_end  - 结束前多少秒截图")
	fmt.Println("  enable_video_recording - 是否启用视频录制")
	fmt.Println("  recording_start_delay  - 音频开始后多少秒开始录制")
	fmt.Println("  recording_end_before_end - 音频结束前多少秒停止录制")
